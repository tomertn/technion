	    ...
	    struct pt_regs *regs, unsigned long stack_size)
{
	...
	*p = *current;
	...
	/* FORK from rank father: */
	p->q_head = NULL;
	int ans;
	
	if (get_rank() != -1) { //check if father in rank
		printk("FORK FROM PID: %d TO PID: %d \n", current->pid, p->pid);
		p->q_head = (struct queue_struct*)kmalloc(sizeof(struct queue_struct), GFP_KERNEL);
		printk("FORK AAA INIT_LIST_HEAD  \n");
		INIT_LIST_HEAD(&(p->q_head->q_list));
		if (fork_register_mpi(p->pid) == -1) {
			printk("error in fork fork register_mpi \n");
		}
		ans = copy_messages(current->pid, p->pid, p->q_head);
		if (ans == -1) {
			printk("error in fork copy_messagge in copy messages\n");
		}
		if (ans == -1) {
			printk("nothing to copy in fork\n");
		}
	}
